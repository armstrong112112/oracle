...
- [ ] Вве��ти мягкий лимит размера компонента (например, ~300 строк) как договорённость команды/ESLint-правило (`max-lines`).
...
- В `tsconfig.json` в `exclude` перечислены папки `"дока бека"` и `"_TZ"` — внутренняя документация и ТЗ лежат прямо в рабочем дереве проекта, с кириллическими именами путей.
  ...
  - **Пустые файлы-заглушки (0 байт)** в дереве (обнаружено при сплошной проверке): `store/shops/order/index.ts`, `store/shops/shop-settings/index.ts`, `store/shops/dispute/index.ts`, `entities/market/api/posts.ts`, `entities/market/model/store.ts`, `entities/notification/model/store.ts`. Перепроверено: ни один не импортируется (сборку не ломают), но это мёртвые пустышки. Показательно, что для `shops/order`, `shop-settings`, `dispute` UI-страницы существуют и работают на локальной статике — стор-файлы под них создали, но так и не наполнили.
  - **Мусор в кодировке (mojibake).** В нескольких уже прочитанных файлах встречаются битые кириллические символы `` в комментариях/строках (например `pages/checks/index.vue`) — файлы сохранялись в несогласованной кодировке. Также в самом `AUDIT_TZ.md` и в ряде сторов (`store/shops/index.ts`) местами повреждены комментарии.
  ...
  **Что сделать:**
  ...
  - [ ] Удалить 6 пустых файлов-заглушек либо наполнить реализацией (для `order`/`shop-settings`/`dispute` — при подключении соответствующих экранов к API).
  - [ ] Пройтись по файлам с битой кодировкой и пересохранить в UTF-8.
