<template>
  <div class="escrow-creating-content">
    <div class="escrow-creating-form">
      <NewEscrowCreating />
    </div>
...
import NewEscrowCreating from "~/entities/escrow/ui/NewEscrowCreating/index.vue";
...
const buttonType = computed(() => {
  const { description, inputAmount } = profile.value;
  const isDataFilled = description != null && inputAmount != null;
...
function handleContinue() {
  if (buttonType.value === "primary") {
    if (props.isModal) {
      ...
      router.push(localePath("/escrow/overview"));
    }
  }
}
...
.escrow-creating-content {
  display: flex;
  flex-direction: column;
  height: 100%;
}

.escrow-creating-form {
  flex: 1;
  padding: 16px 0;
}
