    ...
    <div class="auth-content">
      ...
      <div class="sign_in">
        <div class="flex flex-col gap-12 justify-center items-center">
          <div class="sign_in_title">{{ $t("auth.logInWithTelegram") }}</div>
          <div class="sign_in_description">
            {{ $t("auth.telegramOtpDescription") }}
          </div>
        </div>
        <OTPForm
          ref="otpForm"
...
const verifyOtp = async (otp: string) => {
  try {
    console.log("Verifying Telegram OTP:", otp);
    // Here you would typically verify the OTP with your backend
    // await authStore.registerWithTg({ telegramID: phoneData.fullNumber, code: otp });
    setTimeout(() => {
      router.push(localePath("/auth/sign-up-with-telegram/recovery-phrase"));
    }, 1000);
  } catch (error) {
