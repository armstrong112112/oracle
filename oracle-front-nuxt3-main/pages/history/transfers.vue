<template>
  <history-layout
    ...
    :deposit-title="$t('history.transfers.title')"
    :expenses-title="$t('history.transfers.title')"
...
const actionButtonText = ref<string>('')

// Data
const historyDataRef = historyData
const transferDataRef = transferData
...
const openModal = () => {
  // type, item
  currentModalData.value = { ...transferDataRef }
  showActionButton.value = !!currentModalData.value.actionButtonText
  actionButtonText.value =
    currentModalData.value.actionButtonText === 'Go Back'
      ? t('history.goBack')
      : currentModalData.value.actionButtonText || ''
...
const handleSearchOpenModal = () => {
  // можно внедрить логику поиска позже
}
