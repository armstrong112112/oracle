<template>
  <div class="stats_charts">
    <div class="stats_charts_title" style="text-transform: initial">
      {{ $t("pay.changeAverageIncome") }}
    </div>
    <client-only>
      <apexchart
        type="line"
...
const series = computed(() => [
  {
    name: "BTC",
    data: [75000, 85000, 95000, 90000, 95000, 88000, 90000],
    color: "#FFFFFF",
  },
  {
    name: "USDT",
    data: [70000, 75000, 90000, 80000, 85000, 80000, 95000],
    color: "#FF6B4A",
  },
]);
...
const chartOptions = computed(() => ({
  ...
  tooltip: {
    ...
    custom: ({ series: tooltipSeries, dataPointIndex }: any) => {
      ...
      const usdtValue = tooltipSeries[1][dataPointIndex];

      return `
        <div class="custom-tooltip">
          <div class="tooltip-date">${date}</div>
          <div class="tooltip-value">1 BTC (~${btcValue.toLocaleString()})</div>
          <div class="tooltip-value">9000 USDT (~${usdtValue.toLocaleString()})</div>
