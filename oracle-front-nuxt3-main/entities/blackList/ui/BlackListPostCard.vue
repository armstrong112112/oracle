<template>
  <div ref="postListContainer" class="post-list">
    <div
      v-for="post in posts"
      :key="post.id"
      :ref="post.id === pinnedPostId ? 'pinnedPost' : undefined"
      :class="post.id === pinnedPostId ? 'pinnedPost' : undefined"
      class="post">
      <div
        v-if="post.type === 'text'"
        class="post-content"
        v-html="post.blackListContent"></div>
      <div
        v-else-if="post.type === 'image'"
        class="post-content post-content--media">
        <img
          :src="post.image"
          alt="Post image"
          class="post-image"
          style="height: 164px" />
        <div
          class="post-content post-content--with-media"
          v-html="post.blackListContent"></div>
      </div>
      <div
        v-else-if="post.type === 'video'"
        class="post-content post-content--media">
        <video
          :src="post.video"
          controls
          autoplay
          loop
          class="post-video"></video>
        <div
          class="post-content post-content--with-media"
          v-html="post.blackListContent" />
      </div>
      <div class="post-bottom">
        <div class="post-meta">
          <span>
            <img src="/icons/market/eye.svg" alt="" />
            {{ post.views }}</span
          >
          <span> {{ post.timestamp }}</span>
        </div>
      </div>
      <div class="post-footer">
        <CustomButton value="View Proof" type="primary" class="w-full" />
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { blackListPost } from "~/entities/blackList/data/blackListPost";
import type { Post } from "~/entities/market/model/types";
import CustomButton from "~/components/ui/Button.vue";

interface UpdateReactionsPayload {
  postId: string;
  reactions: { emoji: string; count: number }[];
}

const posts = ref<Post[]>(blackListPost);
const pinnedPostId = ref("3");

const postListContainer = ref<HTMLElement>();
const pinnedPost = ref<HTMLElement[]>([]);

const updateReactions = (payload: UpdateReactionsPayload): void => {
  const post = posts.value.find((p: Post) => p.id === payload.postId);
  if (post) {
    post.reactions = payload.reactions;
  }
};

const scrollToPinnedPost = () => {
  if (
    !postListContainer.value ||
    !pinnedPost.value ||
    pinnedPost.value.length === 0
  ) {
    console.error("Required elements are missing");
    return;
  }

  const pinned = pinnedPost.value[0];
  const container = postListContainer.value;

  pinned.scrollIntoView({ behavior: "smooth", block: "start" });

  const pinnedOffsetTop = pinned.offsetTop - container.offsetTop;
  container.scrollTop = pinnedOffsetTop;
};
</script>

<style lang="scss">
.post-list {
  max-width: 600px;
  margin: 0 auto;
  padding: 20px;
  display: flex;
  justify-content: flex-start;
  align-items: flex-start;
  gap: 12px;
  flex-direction: column;
  position: relative;
  -webkit-overflow-scrolling: touch;
}
.post-content a {
  color: #f64e2a;
}
.post-meta span,
.post-meta svg path {
  color: #8780cf;
}
@media (min-width: 1024px) {
  .post-list {
    width: 100%;
    max-width: 100%;
  }
}

.post {
  background-color: #17161f;
  border-radius: 12px;
  color: #ffffff;
  max-width: 318px;
  width: max-content;
  position: relative;
  &-content {
    font-family: Roboto, sans-serif;
    font-weight: 400;
    font-size: 14px;
    line-height: 130%;
    padding: 8px;

    &--media {
      max-width: 278px;
    }

    &--with-media {
      padding: 0;
      padding-top: 8px;
    }
  }

  &-image,
  &-video {
    max-width: 100%;
    border-radius: 8px;
    object-fit: cover;
    width: 100%;
    height: 100%;
  }

  &-meta {
    color: white;
    opacity: 0.6;
    margin-left: auto;
    text-align: right;
    display: flex;
    justify-content: flex-end;
    font-family: Roboto, sans-serif;
    font-weight: 400;
    font-size: 10px;
    line-height: 130%;
    letter-spacing: 0;

    span {
      display: flex;
      justify-content: center;
      align-items: center;
      gap: 2px;
      margin-left: 6px;
    }
  }

  &-bottom {
    padding: 0 16px;
    display: flex;
    flex-direction: column;
    gap: 12px;
  }
  &-footer {
    font-family: Hanson, sans-serif;
    font-weight: 700;
    font-size: 12px;
    line-height: 100%;
    letter-spacing: 0;
    text-align: center;
    text-transform: uppercase;
    border-top: 1px solid #1d1c28;
    color: #f64e2a;
    padding: 12px;
    cursor: pointer;
  }
}
</style>
